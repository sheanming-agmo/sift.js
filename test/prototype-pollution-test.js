...
describe(__filename + "#", function () {
  ...
  it("skips an inherited $where even when allowStringWhere is false", function () {
    let executed = false;
    global.__proto_pollution_poc = () => {
      executed = true;
    };
    ...
    // The for..in own-key guard runs before `$where` is consulted, so this must
    // neither throw (allowStringWhere:false) nor run the inherited body.
    const result = [{ v: 1 }, { v: 2 }].filter(
      sift({ v: 1 }, { allowStringWhere: false }),
    );
